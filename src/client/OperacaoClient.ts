...
class OperacaoClient {
  ...
  constructor() {
    this.axiosClient = axios.create({
      baseURL: 'http://localhost:8081/api/operacao',
      headers: { 'Content-type': 'application/json' }
    });
  }


  public async findById(id: number): Promise<OperacaoModel> {
    try {
      return (await this.axiosClient.get<OperacaoModel>(`/lista/id/${id}`)).data;
    } catch (error: any) {
  ...
  public async cadastrar(cadastro: OperacaoModel): Promise<string> {
    try {
  ...
  public async editar(id: number, editar: OperacaoModel): Promise<string> {
    try {
...
export default new OperacaoClient();
